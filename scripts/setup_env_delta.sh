...
if [[ "${recreate}" -eq 1 ]]; then
  ...
  # uv sync --extra gpu --extra flash_attn
  ...
  # Set temp dirs
  export TMPDIR="/work/hdd/bchy/shin1/tmp"
  ...
  mkdir -p "$TMPDIR" "$CUDA_CACHE_PATH" "$TORCH_EXTENSIONS_DIR"

  # Ensure correct permissions only if necessary
  # (avoids errors if they already have the correct permissions)
  chmod 777 "$TMPDIR" "$CUDA_CACHE_PATH" "$TORCH_EXTENSIONS_DIR" 2>/dev/null || true

  # 2. Limit ninja concurrency
  export MAX_JOBS=4

  # cd $TMPDIR
  # uv pip install flash-attn --no-build-isolation --python "${SHARED_ENV_DIR}/bin/python"
...
log "Verifying torch and flash_attn imports"
"${ENV_PYTHON}" - <<'PY'
import torch
import flash_attn
