...
@register_dataset("m2sv_sft_dataset")
class M2SVSftDataset(VisionLanguageSftDataset):
    ...
    Example::
        ...
        dataset = M2SVSftDataset(
            hf_dataset_path="yosubshin/m2sv-sft",
            image_map_column="image_map",
            image_sv_column="image_sv",
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if not image_map_column:
            raise ValueError("The `image_map_column` parameter must be provided.")
        if not image_sv_column:
            raise ValueError("The `image_sv_column` parameter must be provided.")
        if not question_column:
            raise ValueError("The `question_column` parameter must be provided.")

        self.image_map_column = image_map_column
        self.image_sv_column = image_sv_column
        ...
        if Path(hf_dataset_path).exists():
            ...
            kwargs["dataset_name"] = "m2sv_sft_dataset"
        else:
    ...
    def transform_conversation(self, example: Union[dict, pd.Series]) -> Conversation:
        ...
        required = {
            "image_map_column": self.image_map_column,
            "image_sv_column": self.image_sv_column,
        ...
        image_map_data = example[self.image_map_column]
        image_sv_data = example[self.image_sv_column]
        question_text = self._process_text_value(example[self.question_column])

        # Create the image content item
        image_map_content_item = self._get_image_content_item(image_map_data)
        image_sv_content_item = self._get_image_content_item(image_sv_data)

        # Create the user message with image and text
        user_message = Message(
            role=Role.USER,
            content=[
                image_map_content_item,
                image_sv_content_item,
